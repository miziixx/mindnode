...
- 빈 공간 단일 탭: 선택 해제 · 빈 공간 더블 탭: 그 위치에 새 노드 추가
- 노드 단일 탭: 선택 · 더블 탭: 편집 하단 시트 · 롱프레스: 편집/복제/연결/삭제 메뉴
- 노드 드래그: 이동 (노드 이벤트를 소비해 캔버스 팬과 충돌하지 않음)
- 두 손가락 핀치: 확대·축소 · 두 손가락 드래그: 캔버스 이동
- 선택 노드의 연결 핸들(코랄 점) 드래그 → 다른 노드 위에서 놓으면 연결
- 노드 편집은 전체 폭 하단 시트, 최소 터치 영역 확보

## Android 연동 (구현됨)

- 가져오기: SAF 문서 선택기(`ACTION_OPEN_DOCUMENT`)로 실제 파일 읽기 + 미리보기·병합/교체
- 내보내기: SAF(`CreateDocument`)로 실제 JSON/Markdown 파일 생성, FileProvider 공유 시트
- 전체 백업 저장으로 앱 삭제 전 백업 가능
- 화면 회전 시 상태 유지(configChanges + ViewModel), 재실행 시 마지막 그래프 복원
- 시스템 뒤로가기: 편집시트 → 메뉴 → 설정 → 드로어 → 연결모드 순으로 닫힘
- API 연결 확인은 실제 요청으로 검증(가짜 성공 메시지 없음)

## 남은 제한사항
1. 디버그 APK는 GitHub Actions에서 빌드되어 Release로 제공됩니다. 이 세션의 원격 환경
   자체는 `dl.google.com` egress 차단으로 로컬 `assembleDebug`를 실행할 수 없습니다.
2. UI/제스처는 CI 빌드로 **컴파일까지 검증**되었으나 **에뮬레이터/실기기 런타임 테스트는
   미수행**입니다. 특히 연결 핸들 드래그의 히트 판정, 핀치줌과 노드 드래그 동시 상황은
   기기에서 확인이 필요합니다.
3. 선택형 AI 노드 생성(빈틈 찾기 등)은 구현되어 있으나 실제 API 키로의 응답 품질은
   제공자·모델에 따라 다릅니다. AI 코드는 로컬 그래프 코드와 분리되어 있어 API 오류가
   편집/저장에 영향을 주지 않습니다.
